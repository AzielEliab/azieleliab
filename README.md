...
| SpectralLock | https://spectrallock-download-tracker.vibelock.workers.dev/ (`GET\|POST /v1/unredact` leftover-bytes recover; opaque rewrite refuses `SL-UNREDACT-OPAQUE`; heatmap ≠ transcript; inject ON is paint not pigment; never OCR-from-black-box; not a FragGate door op) |
...
EmbryoLock Softwares door is catalog `worker_home` (`https://embryolock-download-tracker.vibelock.workers.dev/`). `/embryolock` remains a secondary local page. SpectralLock leftover-bytes honesty (after spectrallock#12 + aziel-runtime#137): leftover container bytes recover (`leftover_bytes`, `recovered_from`); opaque rewrite with nothing left refuses `SL-UNREDACT-OPAQUE` — do not invent letters. Locate / lift / recover / refuse. Heatmap ≠ transcript. Inject ON is paint, not pigment. Worker LIVE: `GET|POST https://spectrallock-download-tracker.vibelock.workers.dev/v1/unredact`. Catalog digest `3427dbcf2932b6bf4c6cf80735efd171b75519066e013db6d0df275c65989fb4`. Unredact is **not** a catalog FragGate door op. FragGate LIVE_OPS stay `health` / `modes` / `targets` / `overlay` / `verify` / `doctor` / `skill`. Hub `/v1/software` inherits live `one_line` / `description`. FragGate, mesh, same-origin `aziel-runtime`, and sister product Trades-Runtime are extras / doors — not Softwares `products[]` cards. Trades-Runtime is local-first BYO field trades (`live_backends` false). `fraggate_call` does not execute company ops. Read-only suite presence is on (display from runtime). GET never enables.
