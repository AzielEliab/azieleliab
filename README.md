...
| `/cite.json` | Citation record (`cold_multi_shelf`, `REDLINE-2026-09-14`, Cap-7 `design_of` + `resolves_to_hub: false` + MirageGrid shuffle LIVE, BAN-SURVIVAL, attack-surface refuse, FoldLock tip-safe) |
...
Same JSON also lives at same-origin `/v1/mesh`, `/v1/mesh/status`, and `/v1/mesh/nodes` (Software door). Live Nodes pull Worker `GET /v1/mesh` (`live_nodes`, `live_nodes_note`, `human_mesh_users`, `human_uses`) via service binding `AZIEL_RUNTIME` (HTTPS origin fallback). **Read-only suite presence is on** — display from runtime. A 404 or missing origin still returns identity Aziel Eliab with `live_nodes: 0` (brandrow/footer show `Live Nodes · 0`; they omit the quiet mesh label rather than an off-state string). Softwares catalog length and `software_nodes` never feed the Live Nodes pill. Mesh ON. Operator-armed Node Gate + neighbor heal + network ON (not a login-recovery / IP panel). AZVPN auto_use + vpn:true (HTTPS/WS REAL; WireGuard/OpenVPN SLOT; GET cites only, never opens a session). Channel plane wifi/bluetooth/rf/photon ON cites; worker_hardware:false. **GET never enables.** Operator enable on runtime requires a declared bearer (example: `suite-presence`). The landing and Softwares-tab brandrow show a Digital Library–style `Live Nodes · N` pill fed from origin `live_nodes` (human mesh users + cited uses). The footer keeps the quiet `mesh on` link when runtime reports enabled, and omits that quiet label when status is unavailable. OpenAPI / cite / llms list the paths. Softwares list rules unchanged. MCP Softwares via `fraggate_call` only; `master_33:false`; FragGate sole door.
