...
Identity is **Aziel Eliab** only. Aka only: Aziel Elroi Eliab, AzielEliab, AzielElroiEliab, The Revealer of The Sealed, Revealer of The Sealed (`alternateName`). Apache-2.0. Person `@id` [https://www.azieleliab.com/#aziel](https://www.azieleliab.com/#aziel).
...
GitHub-side public-website index (GitHub cannot create `AzielEliab/AzielEliab`; case-collides with this Worker): [docs/github-profile-readme/](docs/github-profile-readme/).
...
| `/software` · `/software/` | Distinct Software page (name + designed-purpose one_line). `/#software` maps here. |
...
| `/who` · `/who/` | Visible who-is HTML. H1 `Who is Aziel Eliab` plus the who-answer paragraph. |
...
| `/survival` · `/v1/survival` | BAN-SURVIVAL-1.0 machine pull of runtime `GET /survival` (short TTL). Mutual backup, live_doors, platforms all LIVE, calling-name `*new name alert:`, Cap-7 shuffle via MirageGrid, SPORE-1.0 last-resort failsafe, RE-COLD-STORE (allowed, destinations[], no invent). Cold shelves not marked failed. JSON only — no visible 15:20 chrome. |
| `/cite.json` | Citation record (`cold_multi_shelf`, `REDLINE-2026-09-14`, Cap-7 `design_of` + `resolves_to_hub: false` + MirageGrid shuffle LIVE, BAN-SURVIVAL, SPORE-1.0, RE-COLD-STORE, attack-surface refuse, FoldLock tip-safe) |
...
- Mesh (Live Nodes): https://www.azieleliab.com/runtime/v1/mesh
- Mesh status: https://www.azieleliab.com/runtime/v1/mesh/status
- Mesh nodes: https://www.azieleliab.com/runtime/v1/mesh/nodes
- Survival: https://www.azieleliab.com/runtime/survival (`/v1/survival`)

Same JSON also lives at same-origin `/v1/mesh`, `/v1/mesh/status`, and `/v1/mesh/nodes` (Software door). Dual counter chrome clocks Worker `GET /v1/mesh` `live_nodes` via service binding `AZIEL_RUNTIME` (HTTPS origin fallback). **Read-only suite presence is on** — display from runtime. A 404 or missing origin still returns identity Aziel Eliab with `live_nodes: 0` (brandrow/footer show `0/0`; they omit the quiet mesh label rather than an off-state string). Softwares catalog length and `software_nodes` never feed the pill. Mesh ON. Operator-armed Node Gate + neighbor heal + network ON (not a login-recovery / IP panel). AZVPN auto_use + vpn:true (HTTPS/WS REAL; WireGuard/OpenVPN SLOT; GET cites only, never opens a session). Channel plane wifi/bluetooth/rf/photon ON cites; worker_hardware:false. **GET never enables.** Operator enable on runtime requires a declared bearer (example: `suite-presence`). The landing and Softwares-tab brandrow show `Nodes#/LiveNodes#` next to Views: Nodes = human mesh users + human uses (`j.nodes` preferred; fallback sum or legacy `j.live_nodes` if `nodes` absent); Live Nodes = runtime `GET /v1/mesh` `live_nodes` (human mesh users + site viewers on godlock.uk, azieleliab.com, and azielcorpuslibrary.net). `/count`, `/heartbeat`, and `/v1/mesh` share that read (worker snapshot ≤ 3s, HTTP `no-store`). Local `site_live_nodes` is not added to the pill. After a local human update the hub best-effort POSTs `{ host: "azieleliab.com", viewers, kind: "human-page" }` to runtime `POST /v1/mesh/site-presence` via `AZIEL_RUNTIME` (else `https://aziel-runtime.vibelock.workers.dev`). Bots are not counted. HDJ viewers are excluded. The page POSTs `/heartbeat` on load, every 25s, and when the tab is visible. The footer keeps the quiet `mesh on` link when runtime reports enabled, and omits that quiet label when status is unavailable. OpenAPI / cite / llms list the paths. Softwares list rules unchanged. MCP Softwares via `fraggate_call` only; `master_33:false`; FragGate sole door.
...
- Live Software catalog is **one packed snapshot** (Cache API + KV key `software:catalog:v4` on the existing `VIEWS` namespace). Warm HIT does not fetch runtime. Static `SOFTWARE` remains last resort.
- Soft caps on `/v1/update/check` and mesh origin refresh apply only when someone hammers those fan-out doors after the snapshot is cold. They return the last full JSON (or the quiet pointer / `0/0` body) — not a soft-404, login wall, or thin page. **Rate limit here means cost/abuse protection, not content rationing.**
...
The Software strip (and `/v1/software`, cite, llms, sitemap) prefers the **live** aziel-runtime catalog: packed snapshot first (Cache API / `software:catalog:v4`), then `GET https://aziel-runtime.vibelock.workers.dev/v1/software`, then `GET /v1/fraggate/list`. Same-account service binding `AZIEL_RUNTIME` is tried first. A static slug list remains only as last-resort fallback so the page still renders if runtime is down. New catalog products appear within the snapshot TTL — no hand edit of this repo. Under the Software heading the page lists name + designed-purpose `one_line` from Worker SSoT. `GET /v1/software` keeps catalog fields (`status`, `worker_home`, `version`, `one_line` at minimum) on `products[]`. FragGate and mesh are `extras` only — not Softwares product cards. The product is **aziel-runtime** / **Aziel Runtime**; Software blurbs and meta never mash it as “runtime 1.6.x FragGate”. One public Softwares version: live `GET /v1/software` version is SoT (fallback `/v1/fraggate/list`, then frozen **2.0.0-rc1**). Runtime version cite is **2.0.0-rc1** (or live `GET /v1/health`). SEO abstract still leads. Softwares UI is name + designed-purpose — Runtime distribution buttons live in the `#runtime` card, not under Software.
...
EmbryoLock Softwares door is catalog `worker_home` (`https://embryolock-download-tracker.vibelock.workers.dev/`). `/embryolock` remains a secondary local page. Softwares `one_line` / `description` inherit Worker designed-purpose from live `GET /v1/software` (aziel-runtime #146/#147/#148). Softwares UI lists name + designed-purpose one_line (Plain → Gate → Lock). FragGate, mesh, same-origin `aziel-runtime`, and sister product Trades-Runtime are extras / doors — not Softwares `products[]` cards. Trades-Runtime is local-first BYO field trades (`live_backends` false). `fraggate_call` does not execute company ops. Read-only suite presence is on (display from runtime). GET never enables.
...
Quiet mesh meta: `GET /v1/mesh` (mesh SSoT), `GET /v1/mesh/status`, and `GET /v1/mesh/nodes` (also `/runtime/v1/mesh` · `/runtime/v1/mesh/status` · `/runtime/v1/mesh/nodes`) point at runtime mesh authority. The landing ships `<meta name="aziel-mesh-status">`, `<meta name="aziel-qns-cd">`, `<meta name="aziel-qnm">` (`QNM-BUILD-1.0`), a brandrow `Nodes#/LiveNodes#` pill next to Views, and a muted footer status. Live Nodes is runtime `/v1/mesh` `live_nodes` (fleet). `/count` and `/heartbeat` publish that same number. Local `site_live_nodes` is not added to the pill. Local human updates best-effort POST runtime `/v1/mesh/site-presence`. HDJ excluded. `/v1/software` includes a `mesh` snapshot with `live_nodes` / `live_nodes_note` / `human_mesh_users` / `human_uses` / `site_live_nodes` and a `extras` mesh cite. Softwares count never feeds the pill. **Read-only suite presence is on (display from runtime). GET never enables.** Operator enable requires a declared bearer (example: suite-presence). Operator-armed Node Gate + neighbor heal + network ON (not a login panel). AZVPN auto_use + vpn:true. Channel plane ON cites; worker_hardware:false. No public qnsd proxy. Softwares UI is name + designed-purpose one_line. Try on Glama is the primary Runtime CTA.
...
- Trades-Runtime → https://trades-runtime.vibelock.workers.dev/ — Shadow-first local BYO runtime for HVAC/plumbing/electrical/sewer/cross-trades. BYO ServiceTitan+ProBooks. Human authority. live_backends false. · also MCP https://trades-runtime.vibelock.workers.dev/mcp (one labeled secondary on this door; sister product extra; not a Softwares-tab card; not fraggate_call; live_backends false; public Softwares/cite)
