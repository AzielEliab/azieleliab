...
Person `sameAs` lattice (AZindex): [this hub](https://www.azieleliab.com/), [Corpus](https://www.azielcorpuslibrary.net/), [GodLock](https://godlock.uk/), [He Didn't Jump](https://www.hedidntjump.com/), [GitHub AzielEliab](https://github.com/AzielEliab), [Glama runtime](https://glama.ai/mcp/servers/AzielEliab/aziel-runtime) (owner/repo path — no invented UUIDs). One Person `@id` only. Runtime repo URLs stay on `author` / `creator` / `sourceCode` / `isPartOf`.
