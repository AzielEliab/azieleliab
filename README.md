...
| `GET /v1/mesh` · `/v1/mesh/status` · `/v1/mesh/nodes` | Suite node mesh (read-only suite presence is on; display from runtime GET `/v1/mesh`). `live_nodes` counts human mesh users plus cited human uses. `software_nodes` never feeds Live Nodes. |
...
Same JSON also lives at same-origin `/v1/mesh`, `/v1/mesh/status`, and `/v1/mesh/nodes` (Software door). Dual counter chrome clocks Worker `GET /v1/mesh` (`nodes`, `live_nodes`, `human_mesh_users`, `human_uses`) via service binding `AZIEL_RUNTIME` (HTTPS origin fallback). **Read-only suite presence is on** — display from runtime. A 404 or missing origin still returns identity Aziel Eliab with `live_nodes: 0` (brandrow/footer show `0/0`; they omit the quiet mesh label rather than an off-state string). Softwares catalog length and `software_nodes` never feed the pill. Mesh ON. Operator-armed Node Gate + neighbor heal + network ON (not a login-recovery / IP panel). AZVPN auto_use + vpn:true (HTTPS/WS REAL; WireGuard/OpenVPN SLOT; GET cites only, never opens a session). Channel plane wifi/bluetooth/rf/photon ON cites; worker_hardware:false. **GET never enables.** Operator enable on runtime requires a declared bearer (example: `suite-presence`). The landing and Softwares-tab brandrow show `Nodes#/LiveNodes#` next to Views: Nodes = human mesh users + human uses (`j.nodes` preferred; fallback sum or legacy `j.live_nodes` if `nodes` absent); Live Nodes = presence (`j.human_mesh_users`, or `j.live_nodes` only when `j.nodes` is also present). The footer keeps the quiet `mesh on` link when runtime reports enabled, and omits that quiet label when status is unavailable. OpenAPI / cite / llms list the paths. Softwares list rules unchanged. MCP Softwares via `fraggate_call` only; `master_33:false`; FragGate sole door.
