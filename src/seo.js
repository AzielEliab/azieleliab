...
export function llmsTxt(software = SOFTWARE, survival = null) {
  ...
  return [
    ...
    "- SpectralLock unredact: GET|POST " + SPECTRALLOCK_UNREDACT + " (deep PDF + revision_graph + per-revision copies; not a FragGate door op)",
    "- SpectralLock recover: GET|POST " + SPECTRALLOCK_RECOVER + " (universal artifact recover; NO-LIE; LIVE vs SLOT; not a FragGate door op)",
    "- SpectralLock handwriting: GET|POST " + SPECTRALLOCK_HANDWRITING + " (physical ink scan heuristics; NOT lab/ESDA/court; not a FragGate door op)",
    ...
    "- Suite pack: " + RUNTIME_DOWNLOAD + " (GET /download; not fielded_100)",
    ...
    "- Softwares tab: heading → name list only. Plain A–Z → Gate A–Z → Lock A–Z (Clock ≠ Lock).",
