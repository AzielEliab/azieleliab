...
Read-only suite presence is **on**. Display `live_nodes` from aziel-runtime `GET /v1/mesh` (plane `human-mesh-users-site-viewers`): human mesh users plus site viewers on godlock.uk, azieleliab.com, and azielcorpuslibrary.net. `/count`, `/heartbeat`, and `/v1/mesh` share that field from one runtime read cached at most a few seconds, with `no-store` on the HTTP response. The public pill does not add local `site_live_nodes`. After a local human update the hub best-effort POSTs `{ host: "azieleliab.com", viewers, kind: "human-page" }` to `POST /v1/mesh/site-presence` (`AZIEL_RUNTIME`, else `https://aziel-runtime.vibelock.workers.dev`). Never invent viewers. Bots are not counted. Exclude HDJ. `software_nodes` never feeds Live Nodes. Mesh ON. Operator-armed Node Gate + neighbor heal + network ON (not a login-recovery / IP panel). AZVPN auto_use + vpn:true (HTTPS/WS REAL; WireGuard/OpenVPN SLOT; GET cites only). Channel plane wifi/bluetooth/rf/photon ON cites; worker_hardware:false. Softwares via fraggate_call only; master_33:false; FragGate sole door.

| Path | What |
|------|------|
| `GET /v1/mesh` | Live Nodes SoT. Hoists runtime `live_nodes` (fleet). `site_live_nodes` is this host's presence report and is not added to the pill. Softwares count never feeds the pill. |
