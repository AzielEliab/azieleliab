...
GitHub About / indexing (no Worker UI change): [docs/github-seo.md](docs/github-seo.md). Homepage `https://www.azieleliab.com/`. Cross-link Corpus, GodLock, He Didn't Jump, aziel-runtime, Trades-Runtime (sister product extra — not FragGate exec; `live_backends` false), Try on Glama, Donate.
...
Homepage brandrow shows `Live Nodes · N` (sister-hub pill) from origin `live_nodes` (QNM presence only). Softwares list: live `GET /v1/software` (41 true-engine slugs after main 6a3798a; VeilLock local_only). SpectralLock `one_line` / `description` inherit leftover-bytes honesty after aziel-runtime#137 (digest `3427dbcf2932b6bf4c6cf80735efd171b75519066e013db6d0df275c65989fb4`; overlay digest not rehashed). After spectrallock#13 LIVE (merge `4af8fcb`): leftover container bytes recover; opaque rewrite refuses `SL-UNREDACT-OPAQUE`. Heatmap ≠ transcript. Inject ON is paint, not pigment. Never OCR-from-black-box. Worker LIVE `GET|POST /v1/unredact` (deep PDF + revision_graph + per-revision copies), `GET|POST /v1/recover` (universal artifact recover; NO-LIE; LIVE vs SLOT), `GET|POST /v1/handwriting` (physical ink scan heuristics; NOT lab/ESDA/court) — not FragGate invent. OCR after structural only. In-runtime placements AZVPN / MMConsensus / ToolBench / ZKAttest door to `#task-*`. Heading → list only. Plain A–Z → Gate A–Z → Lock A–Z (StaticClock is Plain). Cap-7 shuffle is MirageGrid LIVE (`https://miragegrid.vibelock.workers.dev` `/v1/health` · `/bridge` · `/v1/shuffle`); `resolves_to_hub: false`. Not a Softwares-tab door.
