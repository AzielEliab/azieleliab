...
describe("SEO routes", () => {
  ...
  it("embeds Person + WebSite + Runtime JSON-LD and a www canonical", () => {
    ...
    assert.equal(ld["@graph"][0].alternateName.length, 3);
    assert.ok(!ld["@graph"][0].alternateName.includes("עזיאל אל ראי אליאב"));
    assert.ok(ld["@graph"][0].disambiguatingDescription.includes("GodLock"));
    assert.ok(ld["@graph"][0].disambiguatingDescription.includes("share only the given name Aziel"));
    assert.ok(!ld["@graph"][0].disambiguatingDescription.includes("1 Chronicles"));
    assert.ok(!ld["@graph"][0].disambiguatingDescription.includes("biblical"));
    assert.ok(ld["@graph"][0].description.includes("living publisher of azieleliab.com"));
...
describe("public entity graph phases B–D + E audit", () => {
  it("locks Person @id to www #aziel and cross-links sameAs", () => {
    ...
    assert.equal(person.alternateName.length, 3);
