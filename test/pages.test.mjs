...
describe("doors", () => {
  ...
  it("lists Trades-Runtime on /doors beside Runtime, linking the giveaway Worker", () => {
    ...
    assert.ok(card);
    const tradesLi = card[0].split("<li>").find((part) => part.includes(">Trades-Runtime<"));
    assert.ok(tradesLi);
    assert.equal((tradesLi.match(/class="also"/g) || []).length, 1);
    assert.match(tradesLi, /class="also">also <a [^>]*href="https:\/\/trades-runtime\.vibelock\.workers\.dev\/mcp"[^>]*>MCP<\/a><\/span>/);
    ...
    assert.ok(card[0].includes('href="' + TRADES_RUNTIME_MCP + '"'));
    assert.ok(card[0].includes(">MCP<"));
    assert.equal(card[0].includes(">" + TRADES_RUNTIME_MCP + "<"), false);
