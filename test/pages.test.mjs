...
describe("SEO routes", () => {
  ...
  it("serves llms.txt, ai.txt, cite.json, sitemap.xml", async () => {
    ...
    assert.ok(llmsBody.includes("default off") || llmsBody.includes("GET never enables"));
  ...
  it("embeds Person + WebSite + Runtime JSON-LD and a www canonical", () => {
    ...
    const catalogApps = ld["@graph"].filter((n) => n["@type"] === "SoftwareApplication" && n.isPartOf);
...
describe("suite node mesh", () => {
  it("defaults off and never claims a hop mesh", () => {
    ...
    assert.equal(liveNodesLabel(null), "Live Nodes · off");
