...
describe("GROKBOT-FIX 1.1 identity lock", () => {
  ...
  it("serves graph.jsonld with Person, five+ FAQ, four hub WebSites, Runtime, Dataset", async () => {
    ...
    assert.equal(WHAT_HE_DOES_SOFTWARES, "Softwares via FragGate (live GET /v1/software).");
  ...
  it("serves who-is text with verbatim answer, software-dev FAQ, 7 rules, Hebrew, stats", async () => {
    ...
    assert.ok(body.includes("PeaceLock — Record chosen silence or chosen inaction as a hash-chained receipt"));
    assert.doesNotMatch(body.split("## Softwares")[1].split("## Research")[0], /THIS-IS-NOT|THIS IS NOT|THIS IS:|never invent letters|verified-status|needs verified/i);
