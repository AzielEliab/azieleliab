...
`person.jsonld` and `identity.jsonld` are the same Person JSON-LD. One Person `@id` only. `graph.jsonld` is Person + FAQPage (primary “Who is Aziel Eliab?” and the software-developer Q lead with GodLock / Aziel Digital Library / aziel-runtime MCP / He Didn't Jump; one concordance FAQ: “Is Aziel Eliab the biblical Aziel from 1 Chronicles?”) + indexable section WebPages (`/mission`, `/software`, …) + WebSite nodes for the four hubs + Runtime `SoftwareApplication` `@id` `https://www.azieleliab.com/runtime#runtime` + Dataset of live stats URLs.

`alternateName` keeps compact Hebrew forms and listed misspellings as AZindex tethers (plus canonical aka). `knowsLanguage` includes `he`. Thin `description` fields lead with the living stack. `disambiguatingDescription` is the sole 1 Chronicles / concordance line on Person — do not copy it into every description. Do not invent biography beyond published About. `sameAs` lattice: azieleliab.com, corpus, godlock.uk, hedidntjump.com, GitHub AzielEliab, Glama runtime. Never sameAs euaziel / Aziel S / Flutter-React. GodLock is a product, not this Person.
