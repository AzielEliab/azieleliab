...
`person.jsonld` and `identity.jsonld` are the same Person JSON-LD. One Person `@id` only. `jobTitle` is researcher, digital rights activist, software developer/designer, author, philosopher of published work. `graph.jsonld` is Person + FAQPage (primary “Who is Aziel Eliab?” plus locked “What does Aziel Eliab do?” / “What Aziel Eliab does” / “Who is Aziel Eliab the developer?” / “What software does Aziel Eliab make?” — Softwares designed-purpose list from Worker GET /v1/software `one_line`, plus The ARK deniable-vault designed-purpose and Whitestone Case Mode designed-purpose, plus SpectralLock designed-purpose FAQ (“What is SpectralLock?” — Preview a 256-pixel overlay, paint membership from the Spectral Harmonic Wheel, and restore faded pigment where the pixels still carry it), plus research MASTER + public hardware halves — plus locked Why FAQ (“Why does Aziel Eliab do this?” / “Why Aziel Eliab” / “Why does Aziel Eliab keep looking?” from published Why + mission goal) — plus “Is Aziel Eliab the two musicians named in 1 Chronicles 15:20?” — both name Aziel and Eliab + verse 15:20; short, no Chronicles essays; plus site-coverage FAQ and Official Aziel ecosystem FAQ: sister sites, GitHub AzielEliab, Try on Glama, X @AzielEliab, Softwares list) + indexable section WebPages (`/why`, `/software`, `/research`, `/doors`, `/aziel`) + WebSite nodes for the four hubs + Runtime `SoftwareApplication` `@id` `https://www.azieleliab.com/runtime#runtime` + Dataset of live stats URLs. Machine site-coverage blurbs cover ae (this hub), corpus library, godlock.uk, hedidntjump.com, and aziel-runtime FragGate/MCP. Official ecosystem cite adds GitHub AzielEliab, Try on Glama, X @AzielEliab, and the Softwares list. PeaceLock designed-purpose: Record chosen silence or chosen inaction as a hash-chained receipt. Trades-Runtime stays extras / sister (software_tab false) with public Softwares/cite. One public Softwares version: live GET https://aziel-runtime.vibelock.workers.dev/v1/software is SoT (fallback /v1/fraggate/list, then frozen 2.0.0-rc1).
